...
# All the results have been drawn on image. Now display the image.
    ...
    if b == c:
        camera=Camera.objects.all()[-1]
        event=Event.objects.create(ou_id=camera.camera_id,event_name=5,event_category=1,model_name="frozen_inference_graph_1.pb",model_version="0.1",confidence_score="99%")
        coordinates={"y1":boxes[0],"x1":boxes[1],"y2":boxes[2],"x2":boxes[3]}
        event_cordinates=Event_Coordinates(**coordinates)
        event_cordinates.save()
        event.event_coordinates=event_cordinates
        event.save()
        data = serializers.serialize("json", event)
        send_data=requests.post("https://cerebra_server/submitEvent/",data=data)
